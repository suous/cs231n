...
def train(
    ...
    weights : ndarray, shape (M, H, W, C)
        The weights of the SVM treated as images for visualization.
    ...
    for i in range(num_iters):
        ...
        loss, dw, db = svm_loss_original_with_grads(x_batch, y_batch, weights, bias, reg)
...
def predict(x: np.ndarray, weights: np.ndarray, bias: np.ndarray) -> np.ndarray:
    """
    Predict the labels of the input images using the trained SVM.
...
def svm_loss_original(x: np.ndarray, y: np.ndarray, weights: np.ndarray, bias: np.ndarray, reg: float) -> float:
    """
    Calculate the loss of the svm using the original image shape.
    ...
    margins = np.maximum(0, s - sy[:, np.newaxis] + 1)
    return np.sum(margins) / num_train - 1 + reg * np.sum(weights * weights)


def svm_loss_reshaped(x: np.ndarray, y: np.ndarray, weights: np.ndarray, bias: np.ndarray, reg: float) -> float:
    """
    Calculate the loss of the svm using the flattened image shape.
    ...
    weights : ndarray, shape (H*W*C, M)
        The weights of the SVM reshaped as a 2d array.
    bias : ndarray, shape (M,)
        The bias of the SVM.
    reg : float
        The regularization strength.
...
def svm_loss_original_with_grads(
    x: np.ndarray, y: np.ndarray, weights: np.ndarray, bias: np.ndarray, reg: float
) -> tuple[float, np.ndarray, np.ndarray]:
    """
    Calculate the loss and gradients of the svm using the original image shape.
    This implementation is slower but more intuitive as it uses the original image to learn the weights as templates.
    ...
    tuple(float, ndarray, ndarray)
        The loss of the SVM, the gradient of the weights, and the gradient of the bias.
    """
    num_train, *_ = x.shape
    s = np.einsum("nhwc,mhwc->nm", x, weights) + bias
    sy = s[np.arange(num_train), y]
    margins = np.maximum(0, s - sy[:, np.newaxis] + 1)
    loss = np.sum(margins) / num_train - 1 + reg * np.sum(weights * weights)

    # Compute the gradient of the weights and bias.
    ds = (margins > 0).astype(int)
    ds[np.arange(num_train), y] -= np.sum(ds, axis=1)
    dw = np.einsum("nm,nhwc->mhwc", ds, x) / num_train + 2 * reg * weights
    db = np.sum(ds, axis=0)
    return loss, dw, db


def svm_loss_reshaped_with_grads(
    x: np.ndarray, y: np.ndarray, weights: np.ndarray, bias: np.ndarray, reg: float
) -> tuple[float, np.ndarray, np.ndarray]:
    """
    Calculate the loss and gradients of the svm using the flattened image shape.
    ...
    ds = (margins > 0).astype(int)
    ds[np.arange(num_train), y] -= np.sum(ds, axis=1)
    dw = x.T @ ds / num_train + 2 * reg * weights
    db = np.sum(ds, axis=0)
    return loss, dw, db
